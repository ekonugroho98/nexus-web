...
function encodeBytes(field: number, bytes: Uint8Array): number[] {
  const tag = (field << 3) | 2;
...
function encodeInt32(field: number, value: number): number[] {
  const tag = (field << 3) | 0;
  return [tag, ...encodeVarint(value)];
}

function encodeNodeTelemetry(field: number, telemetry: any): number[] {
...
function calculateProofHash(proofBytes: Uint8Array): string {
...
export async function generateFibonacciProof(publicInputs: Uint8Array): Promise<Proof> {
...
export async function generateLocalProof(task: Task, publicInputs?: Uint8Array): Promise<Proof> {
...
function encodeTaskRequest(nodeId: string, nodeType: number, ed25519PublicKey: Uint8Array): Uint8Array {
  ...
  const totalSize = 
    2 + nodeIdLength + // field 1 (nodeId): tag + length + data
...
function decodeTaskResponse(buffer: Uint8Array): { taskId: string; programId: string; publicInputs: Uint8Array } {
  ...
  while (offset < buffer.length) {
    ...
    switch (fieldNumber) {
      case 1: // programId (string)
        if (wireType !== 2) break; // length-delimited
...
export async function requestTask(nodeId: string, publicKey: string) {
  try {
...
export async function runFullWorkflow(nodeId: string, publicKey: string): Promise<void> {
  try {
